...
**녹화된 대장내시경 프레임은 모델이 봐야 할 화면과 같은 것이 아닙니다.**
내시경 시야 둘레에는 프로세서 UI 테두리가 있고, 광학계가 닿지 않는 모서리는 까맣고,
letterbox 여백이 붙고, 두 번째 화면을 보여주는 picture-in-picture 창이 뜨기도 합니다.
그 프레임에서 어떤 사각형을 잘라내느냐가 어떤 조직과 수술 도구가 남는지, resize 뒤에
그것들이 얼마나 크게 보이는지, UI 그래픽이 모델까지 들어가는지를 결정합니다. 엄연한
실험 설계 선택인데 보통 논문에 적히지 않습니다. 한 데이터셋에 맞춘 고정 crop은 다음
장비에서 그대로 쓸 수 없고, 시야를 정확히 찾아낸 마스크조차 그 bounding rectangle 안에는
검은 모서리가 그대로 남습니다.
...
논문에 있습니다. 이 저장소가 주는 것은 그 사각형들과, 그것이 어떻게 나왔는지에 대한 기록입니다.
